...
class TMDBAPIManager {
    ...
    func callRequest(query: Int, completionHandler: @escaping ([String]) -> ()) {
        let url = "https://api.themoviedb.org/3/tv/\(query)/season/1?api_key=\(APIKey.tmdbKey)&language=ko-KR"
        ...
        AF.request(url, method: .get).validate().responseData { response in
            ...
            case .success(let value):
                ...
                let stillPathArray: [String] = json["episodes"].arrayValue.map { $0["still_path"].stringValue }
                
                completionHandler(stillPathArray)
    ...
    func requestEpisodeImage() {
        ...
        // 반복문으로 사용하면
        // 1. response 받는 순서를 보장할 수 없음
        // 2. 언제 끝날지 모름 (언제 모든 요청이 처리되는지 확인하기 어려움) -> 다음주에 해결..
        for item in tvList {
            TMDBAPIManager.shared.callRequest(query: item.1) { stillPathArray in
                dump(stillPathArray)
            }
        }
        ...
        TMDBAPIManager.shared.callRequest(query: id) { stillPathArray in
            print(stillPathArray)
